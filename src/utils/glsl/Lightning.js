...
{
float h = clamp(dot(pa, ba) / dot(ba, ba), -0.2, 1.);return length(pa - ba * h);
}
...
void main(void){
    ...
    vec2 tgt = vec2(1., -1.);
    float c = 0.; 
    if (p.y >= 0.) c = (1. - (fbm2((p + .2) * p.y + .1 * iTime)).x) * p.y;
    else c = (1. - (fbm2(p + .2 + .1 * iTime)).x) * p.y * p.y;
    vec3 col = vec3(0.); 
    ...
    col = c * vec3(.7, .8, 1.); }
    col += col1;
    out_FragColor = mix(texture(colorTexture, v_textureCoordinates), vec4(col, 1.0), mix_factor);
}
`;