...
const ImageBasedLighting = Cesium.ImageBasedLighting;
...
//const ModelInstance = Cesium.ModelInstance;
...
function createModel(collection, context) {
  return new Promise(async (resolve, reject) => {
    ...
    if (defined(collection._url)) {
      ...
      collection._model = await Model.fromGltfAsync(modelOptions);
      resolve(collection._model);
    } else {
...
ModelInstanceCollection.prototype.update = function (frameState) {
  ...
  if (this._state === LoadState.NEEDS_LOAD) {
    ...
    createModel(this, context).then((m) => {
      const that = this;
      const model = m;
